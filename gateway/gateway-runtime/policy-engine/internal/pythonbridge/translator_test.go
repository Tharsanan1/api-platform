...
func TestTranslatorToProtoDownstreamMapsTLSFields(t *testing.T) {
	...
	baseTLS := func() *policy.DownstreamTLS {
		return &policy.DownstreamTLS{
			MTLS:                true,
			SHA256Thumbprint:    "deadbeef",
			SubjectDN:           "CN=client-valid",
			FirstURISAN:         "urn:partner-a:payments",
			FirstDNSSAN:         "client-valid.partner-a.test",
			PeerCertificatePEM:  "-----BEGIN CERTIFICATE-----\nMII...\n-----END CERTIFICATE-----\n",
			TLSVersion:          "TLSv1.3",
			RequestedServerName: "api.example.com",
		}
	}

	downstream := func(tls *policy.DownstreamTLS) *policy.DownstreamContext {
		return &policy.DownstreamContext{
			Request: &policy.DownstreamRequest{Path: "/protected", Method: "GET"},
			TLS:     tls,
		}
	}

	t.Run("peer_cert_valid unset (nil)", func(t *testing.T) {
		tls := baseTLS() // PeerCertValid left nil
		result := translator.ToProtoDownstream(downstream(tls))

		require.NotNil(t, result)
		require.NotNil(t, result.GetTls())
		assert.True(t, result.GetTls().GetMtls())
		assert.Equal(t, "deadbeef", result.GetTls().GetSha256Thumbprint())
		assert.Equal(t, "CN=client-valid", result.GetTls().GetSubjectDn())
		assert.Equal(t, "urn:partner-a:payments", result.GetTls().GetFirstUriSan())
		assert.Equal(t, "client-valid.partner-a.test", result.GetTls().GetFirstDnsSan())
		assert.Equal(t, tls.PeerCertificatePEM, result.GetTls().GetPeerCertificatePem())
		assert.Equal(t, "TLSv1.3", result.GetTls().GetTlsVersion())
		assert.Equal(t, "api.example.com", result.GetTls().GetRequestedServerName())
		assert.Nil(t, result.GetTls().PeerCertValid)
	})

	t.Run("peer_cert_valid false", func(t *testing.T) {
		tls := baseTLS()
		valid := false
		...
		require.NotNil(t, result.GetTls())
		require.NotNil(t, result.GetTls().PeerCertValid)
		assert.False(t, result.GetTls().GetPeerCertValid())
	})

	t.Run("peer_cert_valid true", func(t *testing.T) {
		tls := baseTLS()
		valid := true
		tls.PeerCertValid = &valid
		...
		require.NotNil(t, result.GetTls())
		require.NotNil(t, result.GetTls().PeerCertValid)
		assert.True(t, result.GetTls().GetPeerCertValid())
	})
}
...
func TestTranslatorToProtoDownstreamTLSNilWhenGatewayDidNotPopulateIt(t *testing.T) {
	...
	ds := &policy.DownstreamContext{
		Request: &policy.DownstreamRequest{Path: "/unprotected", Method: "GET"},
		TLS:     nil,
	}

	result := translator.ToProtoDownstream(ds)

	require.NotNil(t, result)
	assert.Nil(t, result.GetTls())
}
