...
You are strictly FORBIDDEN from generating a Pull Request description or committing final code until you have executed the following sequence. 

**If you skip this, the CI will fail.**

1.  **Context Switch:** You are likely on the `coverage` image. You MUST switch to the standard image to run local tests.
2.  **Action:** Edit `gateway/it/docker-compose.test.yaml`.
    * Find: `gateway-controller-coverage`
    * Replace with: `gateway-controller`
3.  **Verify:** Run `cd gateway/it && make test`.
4.  **Cleanup:** Revert the image change in `gateway/it/docker-compose.test.yaml` (restore `-coverage`).
